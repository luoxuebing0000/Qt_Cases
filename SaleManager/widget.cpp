...
void Widget::on_sellOkBtn_clicked()
{
    ...
    if (rtn) {
        ...
        QMessageBox::information(this, tr("tips"), tr("pay successful"), QMessageBox::Ok);

    } else {
        QSqlDatabase::database().rollback();
        QMessageBox::information(this, tr("tips"), tr("pay error, can't access database"), QMessageBox::Ok);
    }
    on_sellCancelBtn_clicked();
}
...
void Widget::on_goodsOkBtn_clicked()
{
    ...
    if (rtn) {
        QSqlDatabase::database().commit();
        QMessageBox::information(this, tr("tips"), tr("success"), QMessageBox::Ok);
    } else {
        QSqlDatabase::database().rollback();
        QMessageBox::information(this, tr("tips"), tr("err,can't access database"), QMessageBox::Ok);
    }
    on_goodsCancelBtn_clicked();
}
...
void Widget::on_newOkBtn_clicked()
{
    ...
    if (rtn) {
        ...
        QSqlDatabase::database().rollback();
        QMessageBox::information(this, tr("tips"), tr("error"), QMessageBox::Ok);
    }
...
void Widget::on_changePwdBtn_clicked()
{
    if (ui->oldPwdLineEdit->text().isEmpty() || \
        ...
        QMessageBox::warning(this, tr("warning"), tr("qing jiang xin xi tian xie wanzheng"),QMessageBox::Ok,QMessageBox::Cancel);
    } else {
        QSqlQuery query;
        ...
        if (query.value(0).toString() == ui->oldPwdLineEdit->text()) {
            ...
            if (temp) {
                QMessageBox::information(this, tr("tips"), tr("mima xiugai chenggong"), QMessageBox::Ok);
                ui->oldPwdLineEdit->clear();
                ui->newPwdLineEdit->clear();
            } else {
                QMessageBox::information(this, tr("tips"), tr("mima xiugai shibai"),QMessageBox::Ok, QMessageBox::Cancel);
            }
        } else {
            QMessageBox::warning(this, tr("warning"), tr("yuanmima cuowu,qingchongxin tianxie"),
