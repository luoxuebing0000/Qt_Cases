...
void MySyntaxHighlighterEditor::readSyntaxHighter(const QString &fileName)
{
    ...
    matchReExpression.setPattern(matchReString.trimmed());
...
MyGCodeTextEdit::MyGCodeTextEdit(QWidget *parent):QPlainTextEdit(parent)
{
    ...
    gCodeHighlighter->readSyntaxHighter(QString("../C.txt")); //设置语法高亮文件
...
QString MyGCodeTextEdit::wordUnderCursor() const
{
    ...
    while(curTextCursor.movePosition(QTextCursor::Start, QTextCursor::KeepAnchor, 1))
    {
...
int MyGCodeTextEdit::getBlockCount()
{
    return blockCount();
}
